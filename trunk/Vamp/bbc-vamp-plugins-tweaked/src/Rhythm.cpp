...
Rhythm::FeatureSet Rhythm::getRemainingFeatures() {
  ...
  float firstShift = (int) Helpers::Round(60.f / max_bpm * m_sampleRate / m_stepSize);
  float lastShift = (int) Helpers::Round(60.f / min_bpm * m_sampleRate / m_stepSize);
...
float Rhythm::findRemainder(vector<int> peaks, int thisPeak) {
  ...
  for (unsigned i = 0; i < peaks.size(); i++) {
	...
	total += abs(ratio - Helpers::Round(ratio));
  }
  return total;
}
